...
export default function StepPayment({ booking, bookingId, saving, onPayAtDelivery }: Props) {
  ...
  const termLabel = booking.termType ? t(`config.term.${booking.termType}`) : null;
  const pkgLabel = booking.packageType ? t(`config.pkg.${booking.packageType}`) : null;
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || t("payment.failedStart"));
  ...
  return (
    ...
    >
      <div className="mb-8 md:mb-12">
        <h2 className="text-4xl sm:text-6xl md:text-7xl font-black tracking-tighter text-black uppercase leading-[0.9] mb-4 md:mb-6">{t("payment.title")}</h2>
        <p className="text-black text-sm md:text-lg font-bold uppercase tracking-tight">{t("payment.subtitle")}</p>
      </div>
        ...
        <div className="space-y-4 md:space-y-6 text-[10px] md:text-sm font-black uppercase tracking-tight">
          {pkgLabel && (
            <div className="flex justify-between items-center pb-4 md:pb-6 border-b border-black/10">
              <span className="text-gray-400">{t("payment.package")}</span>
              <span className="text-black">{pkgLabel}</span>
            </div>
          )}
          {termLabel && (
            <div className="flex justify-between items-center pb-4 md:pb-6 border-b border-black/10">
              <span className="text-gray-400">{t("payment.term")}</span>
              <span className="text-black">{termLabel}</span>
            </div>
          )}
          {setupFee > 0 ? (
            <div className="flex justify-between items-center pb-4 md:pb-6 border-b border-black/10">
              <div className="flex items-center gap-2 md:gap-3">
                <span className="text-gray-400">{t("payment.deliverySetup")}</span>
                <span className="text-[8px] md:text-[10px] font-black tracking-widest px-1.5 py-0.5 md:px-2 md:py-1 bg-black text-white">{t("payment.oneTime")}</span>
              </div>
            ...
            <div className="flex justify-between items-center pb-4 md:pb-6 border-b border-black/10">
              <div className="flex items-center gap-2 md:gap-3">
                <span className="text-gray-400">{t("payment.deliverySetup")}</span>
                <span className="text-[8px] md:text-[10px] font-black tracking-widest px-1.5 py-0.5 md:px-2 md:py-1 bg-brutal-green text-black">{t("payment.included")}</span>
              </div>
              <span className="text-brutal-blue">{t("payment.free")}</span>
            </div>
          )}
          <div className="flex justify-between items-center pb-4 md:pb-6 border-b-2 md:border-b-4 border-black">
            <div className="flex items-center gap-2 md:gap-3">
              <span className="text-gray-400">{t("payment.monthlyRental")}</span>
              <span className="text-[8px] md:text-[10px] font-black tracking-widest px-1.5 py-0.5 md:px-2 md:py-1 bg-brutal-blue text-white">{t("payment.recurring")}</span>
            </div>
          ...
          <div className="flex justify-between items-center pt-4 md:pt-6">
            <span className="text-xl md:text-2xl text-black font-black">
              {payAtDelivery ? t("payment.dueAtDelivery") : t("payment.dueToday")}
            </span>
      ...
      <label className="flex items-start gap-4 bg-white p-5 md:p-6 border-3 md:border-4 border-black neo-brutal-shadow mb-8 md:mb-12 cursor-pointer select-none group hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[8px_8px_0px_0px_#000] transition-all">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <span className="font-black text-sm md:text-base uppercase tracking-tight">{t("payment.payAtDelivery")}</span>
          </div>
          <p className="text-[10px] md:text-xs font-bold uppercase tracking-tight text-gray-400 mt-1.5 md:mt-2 leading-relaxed">
            {t("payment.payAtDeliveryDesc")}
          </p>
        </div>
      </label>
      ...
      {payAtDelivery ? (
        ...
        >
          {redirecting || saving ? t("payment.confirming") : t("payment.confirmBooking")}
        ...
        >
          {redirecting ? t("payment.redirecting") : t("payment.paySubscribe")}
      ...
      <div className="mt-8 md:mt-12 flex flex-wrap items-center justify-center gap-x-6 md:gap-x-12 gap-y-3 md:gap-y-4 text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
        <span className="flex items-center gap-1.5 md:gap-2">
          <ShieldCheck className="w-4 h-4 md:w-5 md:h-5 text-brutal-green stroke-[3]" /> {t("payment.secureCheckout")}
        </span>
        {!payAtDelivery && (
          <span className="flex items-center gap-1.5 md:gap-2">
            <Lock className="w-4 h-4 md:w-5 md:h-5 text-gray-400 stroke-[3]" /> {t("payment.poweredByStripe")}
          </span>
        )}
      </div>
    </motion.div>
